...
def timer(cls, func, date_format):
    def wrapped(*args, **kwargs):
        ...
        print(f"Start with '{cls.__name__}.{func.__name__}'. Date and time: {datetime.now().strftime(format)}")
        start = time.time()
        result = func(*args, **kwargs)
        end = time.time()
        print(f"End with '{cls.__name__}.{func.__name__}'. Date and time: {round(end - start, 3)} сек.")
        return result
